...
class Robot(Rectangle):
    ...
    def render(self):
        if self.alive():
            self.health_display = UprightRectangle(self.health_bar.bottom_left.move(0, 1), \
                                                   self.health_bar.width - 1,
                                                   self.health / Robot.health * (self.health_bar.height - 1))
    ...
    def get_enemy(self):
        ...
        robots = enemy.robots
        visible_robots = list(
            filter(lambda enemy: not self.env.is_blocked(LineSegment(self.center, enemy.center), [self, enemy]),
                   robots))
